...
/*   Updated: 2020/12/24 22:30:55 by sgertrud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_escape_sequence(char *command, t_envp *envp, char *(*str), int *i,t_history **history)
{
	...
	if ( !ft_memcmp(tigetstr("kLFT5"),command,ft_strlen(command) + 1))
	{
		//tputs(cursor_invisible, 1, ft_putchar);
		if ((*str)[*i]  == ' ')
		while ((*str)[*i]  == ' ' && *i > 0)
		...
		if ( !ft_memcmp(tigetstr("kRIT5"),command,ft_strlen(command) + 1))
	{
		//tputs(cursor_invisible, 1, ft_putchar);
		if ((*str)[*i]  == ' ')
		while ((*str)[*i]  == ' ' && *i > 0)
	...
	if ( !ft_memcmp(key_up,command,ft_strlen(command) + 1) && (*history)->prev && (*history)->prev->str)
	{
		//if (*str)
		//{
	...
	if ( !ft_memcmp(key_down,command,ft_strlen(command) + 1) && (*history)->next && (*history)->next->str)
	{
		/*if (*str)
		*history = add_history(*history, str);*/
		if	((*history)->next->str)
