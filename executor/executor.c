...
/*   Updated: 2020/12/27 19:21:33 by nelisabe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "executor.h"

static int	run_command(t_commands *commands, t_envp **envp_list, t_exec *exec, int flag)
{
	int		return_value;
	int		mode;
	...
	if (commands->fd_in != -1 && commands->fd_out != -1)
	{
		...
		if ((return_value = \
			...
			return_value = command(commands->command, envp_list, flag);
	}
	else
		return_value = 1;
	if (flag)
...
int			run_commands(t_commands *commands, t_envp **envp, int flag)
{
	t_exec	exec;
	...
	while (commands)
	{
		...
		if ((exec.return_value = set_fd_out(&exec.fd_out, &commands->fd_out)))
			return (error_running(exec.return_value, commands, &exec));
		exec.return_value = run_command(commands, envp, &exec, flag);
